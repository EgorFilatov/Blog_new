...
def parse(request):
    ...
    while i < len(news_list):
        if news_list[-1 - i]['header'] == latest_news:
            break
        news = News(header=news_list[i]['header'], annotation=news_list[i]['annotation'], full_text=news_list[i]['full_text'], category=Categories.objects.get(category='Хабр'), image_url=news_list[i]['image'])
        news.save()
        i = i + 1
    print(latest_news)
